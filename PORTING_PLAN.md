...
**Software status (2026-09-10):** SNTP synchronization against `192.168.1.1`, six-hour resynchronization, monotonic timekeeping between syncs, and `Europe/Brussels` CET/CEST conversion are implemented. Before the first sync, the status bar reports that it is waiting for time and telemetry updates do not initialize hourly buckets. RTC persistence, summary publication, and physical NTP acceptance remain pending; retained summary restore is implemented.
